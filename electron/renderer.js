...
/**
 * Repair Mojibake names where UTF-8 bytes were decoded as Latin-1 ("\u00c3\u00a9" -> "\u00e9").
 * Same replacement table as fixMojibake() in AmuleClient.js — the renderer is
 * context-isolated and cannot require it, so keep the two copies in sync.
 *
 * Names reach us in either Unicode normalization form, so the string is
 * decomposed first and the patterns are matched decomposed ("A" + combining
 * tilde); recomposing at the end leaves already-correct names untouched.
 */
const MOJIBAKE_FIXES = [
  ...
  ["\u00b4", "ô"],
  ["\u0087", "a"],
  ["\u00a0", "à"], // U+00A0 is the real byte; it renders as a space, hence the variant below
  [" ",       "à"],
  ["\u00ae", "î"],
  ["\u0080", "A"],
  ["\u00a7", "ç"],
...
function fixMojibake(str) {
  ...
  let out = str.normalize("NFD");
  for (const [from, to] of MOJIBAKE_FIXES) out = out.replaceAll("A\u0303" + from, to);
  return out.normalize("NFC");
}

/**
 * Fix the names of every row of a list (My Collection, search, discoveries,
 * peer files, peers) in place, so display, filtering and sorting all see the
 * repaired text. The original name is kept in rawFileName — file system
 * operations must keep using it.
 */
function fixListNames(list) {
  for (const r of list || []) {
    if (typeof r?.fileName === "string") {
      if (r.rawFileName == null) r.rawFileName = r.fileName;
      r.fileName = fixMojibake(r.fileName);
    }
    if (typeof r?.userName === "string") r.userName = fixMojibake(r.userName);
    if (typeof r?.peerName === "string") r.peerName = fixMojibake(r.peerName);
  }
  return list || [];
}
...
// Fast tooltips: move button titles into data-tip (rendered by CSS with a short
// delay) so the slow native title tooltip never shows. Runs lazily on hover so
// dynamically rendered rows are covered too.
document.addEventListener("mouseover", (e) => {
  const btn = e.target.closest("button[title]");
  if (!btn) return;
  if (btn.title.trim()) btn.dataset.tip = btn.title;
...
async function loadSharedFiles() {
  try {
    ...
    sharedListCache = fixListNames(list);
...
function renderSharedFiles(list) {
  ...
  sharedBody.innerHTML = list.map((f, i) => {
    ...
    // data-name feeds the file system (open / delete), so it stays the raw name.
    ...
    return `<tr>
      ...
      <td style="white-space:nowrap">
        ...
        <button class="shared-share" data-link="${escapeAttr(link)}" title="Copy ed2k link" aria-label="Share">🔗</button>
        ${path ? `<button class="shared-del danger" data-path="${escapeAttr(path)}" data-name="${escapeAttr(diskName)}" title="Delete file" aria-label="Delete">🗑️</button>` : ""}
      </td>
    </tr>`;
  }).join("");
}
...
async function performSearch() {
  ...
  try {
    ...
    const results = fixListNames(res?.results);
...
function renderDiscoveryResults(results, options = {}) {
  lastDiscoveryResults = fixListNames(results);
...
async function loadPeers() {
  try {
    ...
    fixListNames(peerView.peers);
    fixListNames(peerView.newFiles);
